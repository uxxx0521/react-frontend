...
function Me() {
  ...
  function handleSearchButtonClick() {
    const filtered = entries.filter(
      (entries) => new Date(entries.date).toLocaleDateString("en-CA") === dateInputValue
    );
  ...
  //Get new access token via refresh token API CALL
  let isRefreshing = false;
  const refreshAccessToken = async () => {
    try {
      if (isRefreshing) {
        console.warn("Token refresh is already in progress.");
        return;
      }
      isRefreshing = true;
      ...
      throw error; // Propagate the error to handle logout in the calling function
    }
  };

  // Post user data to server
  const saveUserData = async (retryCount = 0) => {
    try {
      ...
      if (response.status === 403 && retryCount < 1) {
        ...
        if (newAccessToken) {
          // Retry the save request with the new token
          return saveUserData(retryCount + 1);
        }
      }
      ...
      <div className="scroll-container">
        <div className="App-expense-tracker-page">
          <div className="Express-tracker-dashboard">
            ...
            />
            <h1 className="Express-tracker-dashboard-title">Hello {nickName}!</h1>
                  ...
                  <tbody>
                    {(dateInputValue && filteredEntries.length > 0
                      ...
                      <tr key={index}>
                        ...
                        <td>{new Date(entries.date).toLocaleDateString("en-CA")}</td>
                        <td>
